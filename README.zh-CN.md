...
> [!WARNING]
> 风扇控制会通过 MSI ACPI WMI 接口写入固件，兼容性与具体机型密切相关。0.1.0 只会在已经验证的 MSI Cyborg 15 A13VE、WMI 2.8 上启用风扇写入；其他电脑默认进入仅监控模式。
...
- 可编辑的七点温度—风扇曲线
- 普通转速限定为关闭或 30–60%；85°C 保护点固定为全速
- 公开版默认英文，可即时切换简体中文
...
1. 从 GitHub Releases 下载 `MSI-Hardware-Console-v0.1.0-win-x64.zip`。
2. 完整解压压缩包。
3. 运行 `MSIHardwareConsole.exe`，同意 Windows 管理员权限提示。
4. 公开版默认使用英文，点击右上角 **中文** 即可切换。
5. 在已验证机型上，左键点击风扇模式会立即应用；右键点击可查看曲线。
...
- “固定”可持续保持 30–60%，也可明确关闭风扇。
- “自定义”的前六个节点只允许 0% 或 30–60%。
- 最后一个 85°C 节点固定为 100%，并以 3°C 滞回释放 MSI 全速状态。
- “狂暴散热”直接开启固件的最高转速开关。
