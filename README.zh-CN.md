...
> [!WARNING]
> 风扇控制会通过 MSI ACPI WMI 接口写入固件，兼容性与具体机型密切相关。1.0.1 只会在已经验证的 MSI Cyborg 15 A13VE、WMI 2.8 上启用风扇写入；其他电脑默认进入仅监控模式。
...
1. 从 GitHub Releases 下载 `MSI-Hardware-Console-v1.0.1-win-x64.zip`。
2. 完整解压压缩包。
...
- “自定义”的七个节点均可选择 0%、30–60% 或 100%；100% 节点会在对应温度主动启用全速散热。
- 自定义温度节点可设置到 90°C，这是当前机型已经验证的固件写入上限。
- “风扇控制”全部模式之后、“启动与托盘”之前设有同级的“高温保护”板块，包含持续触发（85–95°C，持续 20 秒）、立即触发（90–100°C）和退出保护（70–92°C，持续 20 秒）。
- 三项高温保护恢复为同一行的三张紧凑卡片；绿、橙、红保护节点会接入每条曲线，并始终优先于自定义全速状态。
- 普通与自定义曲线会压缩不可调的 0–30% 和 60–100% 区间但仍保留显示；“狂暴散热”使用完整线性的 0–100% 纵轴。
- 安全约束会保证立即触发温度至少比持续触发高 3°C，退出温度至少比持续触发低 3°C。
- “狂暴散热”直接开启固件的最高转速开关。
