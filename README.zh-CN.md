...
> [!WARNING]
> 风扇控制会通过 MSI ACPI WMI 接口写入固件，兼容性与具体机型密切相关。0.1.3 只会在已经验证的 MSI Cyborg 15 A13VE、WMI 2.8 上启用风扇写入；其他电脑默认进入仅监控模式。
...
1. 从 GitHub Releases 下载 `MSI-Hardware-Console-v0.1.3-win-x64.zip`。
2. 完整解压压缩包。
...
- “风扇控制”标题旁可调整三个高温保护温度：持续触发（88–94°C，持续 20 秒）、立即触发（95–100°C）和退出保护（75–89°C，持续 20 秒）。
