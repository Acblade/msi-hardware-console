...
> [!WARNING]
> 风扇控制会通过 MSI ACPI WMI 接口写入固件，兼容性与具体机型密切相关。0.1.6 只会在已经验证的 MSI Cyborg 15 A13VE、WMI 2.8 上启用风扇写入；其他电脑默认进入仅监控模式。
...
1. 从 GitHub Releases 下载 `MSI-Hardware-Console-v0.1.6-win-x64.zip`。
2. 完整解压压缩包。
...
- “自定义”的七个节点均只允许 0% 或 30–60%；普通曲线不会直接请求 100%。
