...
> [!WARNING]
> Fan control writes to firmware through MSI's ACPI WMI interface. Hardware support is model-specific. Version 1.0.1 enables fan writes only on the verified MSI Cyborg 15 A13VE with WMI interface 2.8; other systems run in monitoring-only mode.
...
1. Download `MSI-Hardware-Console-v1.0.1-win-x64.zip` from GitHub Releases.
...
- All seven `Custom` points allow 0%, 30–60%, or 100%. A 100% point actively engages Full Blast at its selected temperature.
- Custom temperature points can extend through 90°C, which is the verified firmware write limit.
- A dedicated **Thermal guard** section after **Fan control** and before **Startup** contains sustained trigger (85–95°C, 20 seconds), immediate trigger (90–100°C), and release threshold (70–92°C, 20 seconds).
- Thermal guard settings use three compact cards in one row. Their green, orange, and red points are connected into every displayed curve and take priority over Custom Full Blast state.
- Ordinary and Custom charts compress the unavailable 0–30% and 60–100% bands while still showing them; Full Blast uses the full linear 0–100% scale.
...
公开版默认英文，可在右上角即时切换简体中文。1.0.1 版本只确认支持 **MSI Cyborg 15 A13VE、WMI 2.8、单风扇**；其他电脑默认锁定风扇写入，仅提供监控，避免在未验证固件上冒险操作。
