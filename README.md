...
> [!WARNING]
> Fan control writes to firmware through MSI's ACPI WMI interface. Hardware support is model-specific. Version 0.1.4 enables fan writes only on the verified MSI Cyborg 15 A13VE with WMI interface 2.8; other systems run in monitoring-only mode.
...
1. Download `MSI-Hardware-Console-v0.1.4-win-x64.zip` from GitHub Releases.
...
- A dedicated **Thermal guard** panel below **Fan control** contains sustained trigger (88–94°C, 20 seconds), immediate trigger (95–100°C), and release threshold (75–89°C, 20 seconds).
...
公开版默认英文，可在右上角即时切换简体中文。0.1.4 版本只确认支持 **MSI Cyborg 15 A13VE、WMI 2.8、单风扇**；其他电脑默认锁定风扇写入，仅提供监控，避免在未验证固件上冒险操作。
